...
def _generate_member_id(first_name: str, last_name: str, gender: str, phone: str) -> str:
    if not all([first_name, last_name, gender, phone]):
        raise ValueError("First name, last name, gender, and phone are required to generate member ID.")
    phone_suffix = phone[-2:] if len(phone) >= 2 else phone.zfill(2)
    return (first_name[0].upper() + last_name[0].upper() + gender[0].upper() + phone_suffix)
...
def create_member():
    if request.method == 'POST':
        try:
            ...
            city = request.form['city'].strip() or "New Delhi"
            state = request.form['state'].strip() or "New Delhi"
            pincode = request.form['pincode'].strip() or "110019"
            ...
            member_id = _generate_member_id(first_name, last_name, gender, phone)
...
def edit_member(member_id):
    ...
    if request.method == 'POST':
        ...
        member.member_id = _generate_member_id(
            member.first_name,
            member.last_name,
            member.gender,
            member.phone
...
def download_pdf(member_id):
    ...
    if HTML is None:
        return jsonify({
            "message": "PDF generation is unavailable because the WeasyPrint dependency is not installed."
        }), 503
